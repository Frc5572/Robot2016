...
int samp_b[SAMPLES] = { };
int samp_c[SAMPLES] = { };
void writeData(int &fd, int a, int b, int c) {
	samp_a[uptick] = a;
	samp_b[uptick] = b;
	samp_c[uptick] = c;
	uptick++;
	if (uptick == SAMPLES) {
		char hi[25] = { };
		snprintf(hi, sizeof(hi), "S%d:%d:%dE\n", simplp(samp_a), simplp(samp_b),
				simplp(samp_c));
		writeString(fd, string(hi));
		uptick = 0;
...
int main(int argc, char* argv[]) {
	...
	term.c_cflag &=
			~(CLOCAL | CREAD | CSIZE | CSTOPB | PARENB | CRTSCTS | OPOST);
	...
	while (true) {
		...
		findContours(dst, contours, hierarchy, CV_RETR_LIST,
		...
		for (int x = 0; x < contours.size(); x++) {
			...
			Point middle = Point((lbr.x + lbl.x + ltr.x + ltl.x) / 4,
					(lbr.y + lbl.y + ltr.y + ltl.y) / 4);
#ifdef DEBUG
...
#endif
			Point _tl(ltl.x, ltl.y), _tr(contours[x][0].x, contours[x][0].y),
					_bl(contours[x][0].x, contours[x][0].y), _br(
							contours[x][0].x, contours[x][0].y);
			int _ltl = 0;
			int _ltr = 0;
			int _lbl = 0;
			int _lbr = 0;
			...
			Point addTL(max(0, _tl.x - play), _tl.y - play), addTR(_tr.x + play,
					_tr.y - play), addBL(max(0, _bl.x - play), _bl.y + play),
					addBR(_br.x + play, _br.y + play);
			Point subTL(_tl.x + play + tape, _tl.y - play), subTR(
					_tr.x - play - tape, _tr.y - play), subBL(
					_bl.x + play + tape, _bl.y - play - tape), subBR(
					_br.x - play - tape, _br.y - tape - play);
			positive = !((_tl.x <= 0 && _tl.y <= 0)
					|| (_tr.x <= 0 && _tr.y <= 0) || (_br.x <= 0 && _br.y <= 0)
					|| (_bl.x <= 0 && _bl.y <= 0));
			for (int y = 0; y < contours[x].size(); y++) {
				Point w = contours[x][y];

				if (!PointInQuad(w, addTL, addTR, addBR, addBL)
						|| PointInQuad(w, subTL, subTR, subBR, subBL)) {
					positive = false;
#ifdef DEBUG
			...
			if (positive) {
				...
				distance = sqrt(
						pow(
								((21834.83 / ((finalwidthb + finalwidtht) / 2)
										+ 135)), 2) - 7921) - 144;
		...
		if (positive) {
			...
			writeData(fd, (int) finalMid.x, width, (int) finalMid.y);
