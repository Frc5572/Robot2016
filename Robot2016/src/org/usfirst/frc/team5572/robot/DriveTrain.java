...
public class DriveTrain {
	...
	public static boolean driveStraight(double speed, double thresh,
			double dist_in) {
		double dist = dist_in * 11.0107526882/* * .965517 */;
		if (Snoopr.getRightEncoderRaw() <= dist - thresh) {
			double delta = Snoopr.getRightEncoderRaw()
					- Snoopr.getLeftEncoderRaw();
			double modDelta = delta /100d;
			double speed2 = clamp(0.002 * (dist - Snoopr.getRightEncoderRaw()),
					.2, speed);
	...
	public static boolean setGlobalAngle(double angle, double thresh,
		...
		if (Math.abs(angle - curr) <= thresh) {
			...
			accum = 0;
			if (Math.abs(turn(signum(angle - curr) * (90 * (min_power - 0.05))
					+ angle - curr)) < min_power)
				return true;
		}
		return false;
	}
